...
<%- include('../partials/head.ejs', {page: user.name, vue: true, profile:true, username:user.name}); %>
...
<body class='bg-gray-100'>
    ...
    <main class='container mx-auto px-4 mt-24 md:mt-16' id='app' v-cloak>
        ...
        <div v-for="post in posts" class="max-w-2xl flex p-6 mx-auto my-5 bg-white rounded-lg shadow-md" v-bind:class="{ 'bg-indigo-100': post.highlight }">
            <div class="pt-1">
                <img v-bind:src="'/picture/'+post.poster" height="30px" class='rounded-full h-6 inline-block shadow'>
                ...
                <p class="text-base text-gray-500 italic leading-normal">
                    ...
                    - {{ post._id }}</p>
                <div>
                    <span @click="lovePost(post._id)" tabindex="0" class="love"
                        v-bind:class="{ 'text-red-600': post.loves.includes(loggedInUserID) }">
                        <span v-bind:data-post-id='post._id'
                    ...
                    <span v-bind:data-post-count-id='post._id'>{{ post.loves.length }}</span>
                    <!-- 
